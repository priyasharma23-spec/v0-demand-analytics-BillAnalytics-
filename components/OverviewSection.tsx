...
export default function OverviewSection({ appState, onStateChange, onBranchChange, onCAChange, onSectionChange, onHeatmapCellClick, onAnomalyClick }: OverviewSectionProps) {
  ...
  return (
    <div style={{ padding: '20px', background: '#f5f6fa', minHeight: '100vh' }}>
      ...
      {/* Pinned + Recent entities row — simplified */}
      <div style={{
        ...
        {/* PINNED label */}
        <span style={{
          fontSize: '12px', fontWeight: 600, color: '#9b9b96',
        ...
        {/* Pinned chips — simple dots + names */}
        ...
        {/* Recent chips — simple dots + names */}
        {visibleRecent.map(e => (
          <div
            key={e.name}
            style={{
          ...
          >
            ...
            <div style={{
              width: '6px', height: '6px', borderRadius: '50%',
              background: dotColor(e.type), flexShrink: 0, opacity: 0.7,
            }} />
            {/* Name */}
            <span>{e.name}</span>
          </div>

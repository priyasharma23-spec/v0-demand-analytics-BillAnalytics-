  ...
  return (
    <div style={{ background: '#f0f5fa', padding: '20px' }}>
      ...
      {(() => {
        ...
        const caSchedule = allCAsLocal.map((ca) => {
            ...
            <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: '16px', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px 14px', borderBottom: '1px solid #F3F4F6', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ background: '#EEF2FF', border: '1.5px solid #C7D2FE', borderRadius: '10px', padding: '6px 14px', textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: '18px', fontWeight: 800, color: '#4338CA' }}>{inr(totalUnpaidLocal)}</div>
                </div>
              </div>
              ...
              {activeWeek !== null && weeklyAmounts[activeWeek] && (
                <div style={{ margin: '0 12px 14px', background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: '10px', padding: '12px 16px' }}>
                  <div style={{ fontSize: '11px', color: '#6B7280', marginBottom: '8px' }}>{weeklyAmounts[activeWeek].label} detail</div>
                  <div style={{ display: 'flex', gap: '20px' }}>
                    {([['Total due', inr(weeklyAmounts[activeWeek].unpaid)],['Overdue', weeklyAmounts[activeWeek].overdue > 0 ? inr(weeklyAmounts[activeWeek].overdue) : '—'],['Bills', weeklyAmounts[activeWeek].unpaidCount+' unpaid']] as [string,string][]).map(([label,value]) => (
                      <div key={label}><div style={{ fontSize: '10px', color: '#6B7280', marginBottom: '2px' }}>{label}</div><div style={{ fontSize: '13px', fontWeight: 700, color: '#111827' }}>{value}</div></div>
                    ))}
                  </div>
                </div>
              )}
